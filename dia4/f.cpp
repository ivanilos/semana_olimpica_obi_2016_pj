...
int solve(int x) {
	if (x < 0) { // retornar um valor em modulo grande para nao pegar esse valor
		return -INF;
	}
	if (x == 0) { // se nao ha mais fita, fizemos um corte correto, retornar 0
		return 0;
	}
	if (dp[x] != -1) { // se ja calculamos o valor, retornar ele, nao calcular de novo
		return dp[x];
	}
	int aux = -INF;
